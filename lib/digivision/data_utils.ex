defmodule Digivision.Data_Utils do
  ...
  def dataset_split(price_dataset, split_ratio) when split_ratio > 0.0 and split_ratio < 1.0 do
    dataset_count = Enum.count(price_dataset)
    train_count = round(split_ratio * dataset_count)
    # after some prep, finally split the data
    {Enum.take(price_dataset, train_count), Enum.drop(price_dataset, train_count)}
  end
